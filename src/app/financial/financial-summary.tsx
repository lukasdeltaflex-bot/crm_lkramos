...
export function FinancialSummary({ rows, currentMonthRange, isPrivacyMode, onShowDetails, userSettings }: FinancialSummaryProps) {
  const {
    ...
    rows.forEach(p => {
        ...
        const isDeleted = p.deleted === true; // IGNORA ITENS NA LIXEIRA
    ...
    const productionTrend = last7Days.map(day => {
        ...
        return rows.reduce((sum, p) => {
            ...
            // 🛡️ IGNORA REPROVADOS E EXCLUÍDOS TAMBÉM NO SPARKLINE
            return (d && d >= ds && d <= de && p.status !== 'Reprovado' && p.deleted !== true) ? sum + safeValue(p.commissionValue) : sum;
        }, 0);
    });
    ...
    const receivedTrendData = last7Days.map(day => {
        ...
        return rows.reduce((sum, p) => {
            ...
            return (d && d >= ds && d <= de && p.deleted !== true && (p.commissionStatus === 'Paga' || p.commissionStatus === 'Parcial')) 
